    ...
    public View getView(int position, View item, @NonNull ViewGroup parent) {
        ...
        if (mData.size() > position) {
            ...
            switch (wrapper.matchType) {
                case NAME:
                    holder.nameView.setText(Strings.getHighlighted(name, match, HIGHLIGHT_COLOR, true));
                    holder.cnView.setText(prescription.getCode());
                    break;
                case CODE:
                    holder.cnView.setText(Strings.getHighlighted(prescription.getCode(), match, HIGHLIGHT_COLOR, true));
